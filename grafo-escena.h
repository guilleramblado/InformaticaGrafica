...
//EJERCICIO ADICIONAL Nº 1 DE LA PRACTICA 3


class GrafoEstrellaX : public NodoGrafoEscena {
   protected:
      //puntero a la matriz de transformacion afectada por dicho grado de libertad...
      glm::mat4 *pm_rotacion = nullptr;
   
   public:
      GrafoEstrellaX(unsigned n,const float angulo_rot_inicial);
      void fijar_angulo(const float nuevo_angulo);
...
class GrafoCubos: public NodoGrafoEscena {
   protected:
      glm::mat4 *pm_rotacion = nullptr;
   public:
      GrafoCubos(const float angulo_rotacion);
      virtual unsigned leerNumParametros() const;
      void fijar_angulo(const float nuevo_angulo);
      virtual void actualizarEstadoParametro( const unsigned iParam, const float t_sec );
};
