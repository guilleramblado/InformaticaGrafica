...
void DescrVAO::crearVAO()
{
   ...
   for(int i=0;i<dvbo_atributo.size();i++){
      if(dvbo_atributo[i]){
         //el puntero verdaderamente apunta a un objeto VBO de atributo, lo creamos
   ...
   for(int i=0;i<dvbo_atributo.size();i++){
      //comprobamos si elpuntero apunta a un vbo
      if(dvbo_atributo[i] && !atrib_habilitado[i]){
         glDisableVertexArrayAttrib(array,i);
      }
   }
   //
...
void DescrVAO::draw( const GLenum mode )
{
   ...
   //
   if(array==0){
      //no se ha creado el VAO
      crearVAO();
   }
   else{
      //esta yá creado, lo activamos
      glBindVertexArray(array);
   }
